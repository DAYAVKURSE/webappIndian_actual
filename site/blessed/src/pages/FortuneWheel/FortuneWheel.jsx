...
export const FortuneWheel = () => {
  ...
  useEffect(() => {
    ...
    // const wsUrl = `wss://176.120.21.195:8080/ws/fortunewheel/live?init_data=${encoded_init_data}`;
    const wsUrl = `wss://176.120.21.195:8080/ws/fortunewheel/live`;
    ...
    const connectWebSocket = () => {
        ...
        try {
          ...
          setWins((prevWins) => {
            if (
              !prevWins.some(
                (win) =>
                  win.nickname === newWin.nickname && win.amount === newWin.amount && win.timestamp === newWin.timestamp
              )
            ) {
      ...
      ws.onclose = () => {
        if (retries < maxRetries) {
          retries++;
          console.warn(`WebSocket closed. Retrying (${retries}/${maxRetries})...`);
          setTimeout(connectWebSocket, 1000 * retries);
        } else {
          console.error('WebSocket closed permanently after max retries.');
        }
      };
    };
      ...
      <div className={styles.fortunewheel__balance}>
        ...
        <div className={styles.fortunewheel__block}>
          <p className={styles.fortunewheel__balance__value}>
            ...
            {fraction && <span className={styles.fortunewheel__balance__value_fraction}>.{fraction}</span>}
          </p>
          <img
            className={`${styles.fortunewheel__balance__value_img} ${
              fraction ? styles.fortunewheel__balance__value_img_fraction : ''
            }`}
