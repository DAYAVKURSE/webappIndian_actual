...
export const Crash = () => {
  const { BalanceRupee, increaseBalanceRupee, decreaseBalanceRupee } =
    useStore();
    ...
    const fetchHistory = async () => {
      try {
        ...
        if (data && data.results) {
          ...
          if (lastResult) {
            valXValut.current = parseFloat(
              lastResult.CrashPointMultiplier.toFixed(2)
            );
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(
      `wss://testfakeserver.com/api/ws/crashgame/live?init_data=${encoded_init_data}`
    );
    wsRef.current = ws;
    ...
    ws.onmessage = async (event) => {
      try {
        ...
        if (data.type === "game_crash") {
          ...
          setTimeout(() => {
            if (bet > 0) {
              ...
              toast.error(
                `Game crashed at ${data.crash_point.toFixed(
                  2
                )}x! You lost ₹${bet}.`
              );
              setBet(0);
        ...
        if (data.type === "timer_tick") {
          ...
          if (data.remaining_time > 13) {
            ...
            if (queuedBet > 0) {
              ...
              try {
                const response = await crashPlace(
                  queuedBet,
                  autoOutputCoefficient
                );
                if (response.ok) {
                  setBet(queuedBet);
        ...
        if (data.type === "cashout_result") {
          ...
          toast.success(
            `You won ₹${data.win_amount.toFixed(0)}! (${
        ...
        if (data.type === "other_cashout") {
          toast.success(
            `${data.username} won ₹${data.win_amount.toFixed(0)} at ${
              data.cashout_multiplier
            }x!`
          );
        }
  ...
  return (
    <div className={styles.crash}>
      ...
      <div className={styles.crash_wrapper} ref={crashRef}>
        <div
          className={`${styles.crash__collapsed} ${
            collapsed ? styles.fadeIn : styles.fadeOut
          }`}
        >
          <p>{overlayText}</p>
        </div>
        ...
        {bet > 0 && !isCrashed && (
          <div className={styles.activeBet}>Your bet: ₹{bet}</div>
        )}
        {queuedBet > 0 && (
          <div className={styles.queuedBet}>Queued bet: ₹{queuedBet}</div>
        )}
      </div>
      ...
      <div className={styles.betSection}>
        <div className={styles.coefficientContainer}>
          <div className={styles.coefficientLabel}>
            Coefficient
            <button
              className={`${styles.autoCashoutBtn} ${
                isAutoEnabled ? styles.active : ""
              }`}
        ...
        <div className={styles.betControls}>
          ...
          <div className={styles.quickButtons}>
            <button
              ...
              onClick={() => handleMultiplyAmount(0.5)}
            >
              /2
            </button>
            <button
              className={styles.quickButton}
              onClick={() => handleMultiplyAmount(2)}
            >
              x2
            </button>
          </div>
          ...
          {bet > 0 ? (
            <button
              className={`${styles.mainButton} ${
                gameActive && !isCrashed ? styles.activeButton : ""
              }`}
            ...
            <button
              className={`${styles.mainButton} ${
                isBettingClosed ? styles.queuedButton : ""
              }`}
              onClick={handleBet}
